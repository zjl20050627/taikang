...
class ContextBuilder:
    ...
    RELATION_TEMPLATES = {
        ...
        "属于":     "{head}属于{tail}类疾病",
        "BELONGS_TO": "{head}属于{tail}",
        "TREATED_BY": "{head}的常用治疗药物包括{tail}",
        "INDICATED_FOR": "{tail}适用于{head}的治疗",
        "并发症":   "{head}可能引发的并发症包括{tail}",
        "需要检查": "{head}患者需要进行{tail}",
        "除外疾病": "{head}的除外责任中包含{tail}（即患有该病可能不予承保）",
        "EXCLUDES": "{head}的除外责任中包含{tail}（即患有该病可能不予承保）",
        "可承保":   "{head}可以承保{tail}患者",
        "COVERS":   "{head}可以承保或覆盖{tail}",
        "年龄限制": "{head}的投保年龄范围为{tail}",
        "HAS_AGE_LIMIT": "{head}的投保年龄范围为{tail}",
        "保障内容": "{head}的保障内容包括{tail}",
        "COVERAGE": "{head}的保障内容包括{tail}",
        "年保费":   "{head}的年保费大约为{tail}",
        "PRICE":    "{head}的年保费大约为{tail}",
        "提供服务": "{head}提供{tail}服务",
        "PROVIDES": "{head}提供{tail}服务",
        "收费标准": "{head}的收费标准约为{tail}",
        "CHARGE":   "{head}的收费标准约为{tail}",
        "接收条件": "{head}的入住/接收条件为{tail}",
        "ADMISSION": "{head}的入住/接收条件为{tail}",
        "保障疾病": "{head}的保障范围覆盖{tail}",
        "治疗方案": "{head}的推荐治疗方案包括{tail}",
        "PARTNER_WITH": "{head}与{tail}存在保险+养老协同关系",
    }
